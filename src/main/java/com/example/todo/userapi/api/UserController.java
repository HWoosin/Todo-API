...
public class UserController {
    ...
    public ResponseEntity<?> promote(
        ...
        catch (IllegalStateException | NoRegisteredArgumentException e){
            e.printStackTrace();
    ...
    public ResponseEntity<?> loadFile (
        ...
        try {
            ...
            if(contentType == null){
                return ResponseEntity.internalServerError().body("발견된 파일은 이미지 파일이 아니다.");
            }
            headers.setContentType(contentType);//사용자가 여러타입을 가져올수있다.
    ...
    private MediaType findExtensionAndGetMediaType(String filePath) {//확장자의 타입에 따라 미디어타입리턴하는 함수

        //파일 경로에서 확장자 추출하기
        //C:/todo_upload/asdfasdsdfasdgag-abc.jpg
